...
The binary has the frontend compiled into it, so a release carries other
people's software as well: Angular, uPlot, Transloco, the IBM Plex
fonts, the MySQL driver and a handful of Go libraries. Their licences
are reproduced in [THIRD-PARTY-NOTICES.md](THIRD-PARTY-NOTICES.md),
which is generated by `tools/licenses/collect.sh` and checked by CI, so
a new dependency cannot quietly arrive without its notice.
